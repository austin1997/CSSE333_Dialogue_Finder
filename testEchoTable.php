			...
			<table class="mdl-data-table mdl-js-data-table mdl-shadow--2dp">
				...
				<tbody>
					<?php
						...
						if(!empty($animeName)){
							$posts = sqlsrv_query($conn, "EXEC search_Anime N'$animeName'");
							if (!($posts) || $posts == 1) {
							...
							$posts = sqlsrv_query($conn, "EXEC search_Anime");
							if (!($posts) || $posts == 1) {
								echo 'Sorry! There is no anime existed.';
							}else{
								while ($row = sqlsrv_fetch_array($posts))	{
									echo '<tr><td class="mdl-data-table__cell--non-numeric">'.$row[0].'</td><td>'.date_format($row[1], 'd/m/y').'</td><td>'.$row[2].'</td><td>'.$row[3].'</td></tr>';
								}
				...
				<tbody>
					<?php
						...
						if(!empty($fromAnimeName)){
							$posts1 = sqlsrv_query($conn, "EXEC search_Episode N'$fromAnimeName'");
					//		echo 'posts are: '.$posts1.'<br>';
							...
							$posts1 = sqlsrv_query($conn, "EXEC search_Episode");
					//		echo 'posts are: '.$posts1.'<br>';
							if (!($posts1)) {
								echo 'The anime you searched is not available';
							}else{
				...
				<tbody>
					<?php
						...
						if(!empty($dialogEpisode)){
							...
							if (!($posts2)) {
								echo 'Matched dialogue not found';
							}else{
							...
							if(!empty($dialogKW)){
								$posts2 = sqlsrv_query($conn, "SELECT Time, Content_JP, Content_CHN FROM Dialogue WHERE Content_JP LIKE N'%$dialogKW%'");
								echo 'posts are: '.$posts.'<br>';
								if (!($posts2)) {
									echo 'Matched dialogue not found here';
								}else{
									while ($row = sqlsrv_fetch_array($posts2))	{
										echo '<tr><td class="mdl-data-table__cell--non-numeric">'.date_format($row[0], 'H:i:s').'</td><td>'.$row[1].'</td><td>'.$row[2].'</td></tr>';
									}
								...
								$posts2 = sqlsrv_query($conn, "EXEC search_Dialogue");
								//echo 'posts are: '.$posts.'<br>';
								if (!($posts2)) {
									echo 'Sorry! There is no dialogue existed.';
								}else{
									while ($row = sqlsrv_fetch_array($posts2))	{
										echo '<tr><td class="mdl-data-table__cell--non-numeric">'.date_format($row[0], 'H:i:s').'</td><td>'.$row[1].'</td><td>'.$row[2].'</td></tr>';
									}
